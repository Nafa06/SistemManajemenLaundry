...
el('#loginBtn').onclick = async () => {
    ...
    // Bypass Khusus Demo Admin Internal
...
el('#logoutBtn').onclick = () => { 
    localStorage.removeItem('SIML_user'); 
    location.reload(); 
};
...
async function loadSession(){
    ...
    if(currentUser.role === 'Admin'){
        ...
        // Auto isi nama & kontak di form input cucian
...
if(el('#photoUpload')){
    el('#photoUpload').onchange = (e) => {
        ...
        reader.onloadend = async () => {
            ...
            // Opsional: fetch PUT ke API untuk simpan permanen di Neon
        };
...
/* ==========================================
   5. LOGIKA DATA LAUNDRY (API KE NEON)
   ========================================== */
...
async function renderMyActive(){
    if(!currentUser) return;
    el('#myActiveTransactions').innerHTML = '<div class="skeleton h-16 w-full rounded-2xl"></div>'; // Skeleton
...
async function renderHistory(){
    el('#historyTableWrap').innerHTML = '<div class="skeleton h-32 w-full rounded-2xl"></div>';
    const all = await fetchTrans();
...
window.openPayModal = function(id, total){
    ...
    el('#modalPayTotal').textContent = fmtIDR(total); 
...
el('#closePayModal').onclick = () => { 
    el('#payModal > div').classList.add('scale-90'); 
    setTimeout(()=>el('#payModal').classList.add('hidden'), 300); 
};
...
window.fillAdminInputs = function(id){
    ...
    el('#adminNewStatus').value = 'NoChange';
...
async function renderAdminPayment(){
    ...
    el('#pendingCount').textContent = pend.length;
    el('#confirmedCount').textContent = all.filter(x=>x.status_payment==='Lunas').length;
    
    el('#adminPendingList').innerHTML = pend.length ? pend.map(x=>`
        <div class="border rounded-2xl p-4 flex justify-between items-center bg-gray-50">
